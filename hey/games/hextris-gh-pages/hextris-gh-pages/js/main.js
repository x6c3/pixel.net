...
function checkGameOver() {
    for (var i = 0; i < MainHex.sides; i++) {
        if (isInfringing(MainHex)) {
            ...
            // Check if the score is a high score and add it to the highscores array if necessary
            if (highscores.indexOf(score) == -1) {
                highscores.push(score);
            }

            // Write the high scores to some storage (not shown in your code)
            writeHighScores();

            // Redirect to the next page with parameters
            redirectToNextPage(score, highscores[0]); // Pass the high score and current high score
            return true;
        }
    }
    return false;
...
// Function to redirect to the next page with parameters
function redirectToNextPage(currentScore, highScore) {
